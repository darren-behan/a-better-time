  ...
  function updateArray(sender) {
    ...
    if (sender == 2) { thisArray = tripAdvisor; }
      ...
      for (i = 0; i < thisArray.length; i++) {
        ...
        //   timeout: 0,
        //   success: function (response) {
        //     if (response.results[0].locations[0].latLng.lat && response.results[0].locations[0].latLng.lng) {
        ...
        //       // instead of passing i and sender I would do this
        //       // thisArray[i].d = calculateAndUpdate(theLat, theLng);

        //       calculateAndUpdate(theLat, theLng, i, sender);

        //     } else {
        //       console.log('failed to get lat/lon');
...
// the object
  ...
  getGeoLocations(function () {
    ...
    // 1.
    // we can chain functions if they returns promises,
    // each functions were changed to return $.ajax

/*
    // XXX the main problem is error handling
    zomatoAPI().then(tripAd).then(ticketMaster).then(function () {
      // if we are here then we could managed to run zomato, tripAt and ticketMaster one by one order
